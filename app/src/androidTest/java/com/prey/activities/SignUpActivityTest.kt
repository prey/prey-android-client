...
import com.prey.activities.SignUpActivity.Companion.SIGN_UP_ACTIVITY_FORM
import com.prey.net.TestWebServices
import com.prey.R
import com.prey.activities.SignUpActivity.Companion.SIGN_UP_ACTIVITY_ERROR
...
class SignUpActivityTest {
    ...
    lateinit var scenario: ActivityScenario<SignUpActivity>
    ...
    @Test
    fun test_signUpActivity_withWebForm() {
        ...
        assertEquals(activityView, SIGN_UP_ACTIVITY_FORM)
    }

    @Test
    fun test_signUpActivity_button_empty() {
        PreyConfig.getInstance(context).setApikey("")
        PreyConfig.getInstance(context).setDeviceId("")
        ...
        val activityView = PreyConfig.getInstance(context).getActivityView()
        assertNotNull(activityView)
        assertEquals(activityView, SIGN_UP_ACTIVITY_ERROR)
    }

    @Test
    fun test_signUpActivity_button_ok() {
        PreyConfig.getInstance(context).setApikey("")
        PreyConfig.getInstance(context).setDeviceId("")
        PreyConfig.getInstance(context).setWebServices(TestWebServices())
        scenario = ActivityScenario.launch(withWebFormIntent(context))
        ...
        val activityView = PreyConfig.getInstance(context).getActivityView()
        assertNotNull(activityView)
        // assertEquals(activityView, SIGN_UP_ACTIVITY_WELCOME)
