...
@LargeTest
class PasswordNativeActivityTest {

    private val context: Context = ApplicationProvider.getApplicationContext()
    lateinit var scenario: ActivityScenario<PasswordNativeActivity>

    private fun withWebFormIntent(context: Context): Intent {
        val intent = Intent(context, PasswordNativeActivity::class.java)
        return intent
    }

    @Test
    fun test_password_with_lock() {
        ...
        assertEquals(activityView, PasswordNativeActivity.PASSWORD_OK)
    }

    @Test
    fun test_password_with_empty() {
        PreyConfig.getInstance(context).setApikey("")
        PreyConfig.getInstance(context).setDeviceId("")
