...
@LargeTest
class PreyConfigurationActivityTest {

    private val context: Context = ApplicationProvider.getApplicationContext()
    lateinit var scenario: ActivityScenario<PreyConfigurationActivity>

    private fun withWebFormIntent(context: Context): Intent {
        return Intent(context, PreyConfigurationActivity::class.java)
    }

    @Test
    fun test_configuration_withWebForm() {
        PreyConfig.getInstance(context).setApikey("")
        PreyConfig.getInstance(context).setDeviceId("")
        PreyConfig.getInstance(context).setTest(true)
        ...
        assertEquals(activityView, PreyConfigurationActivity.ACTIVITY_FORM)
    }
