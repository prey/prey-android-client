...
import com.prey.net.http.kotlin.SimpleMultipartEntity
...
class UtilConnection {
            ...
            do {
                ...
                if (!isInternetAvailable()) {
                    ...
                    if (entityFiles != null && entityFiles.size > 0) {
                        for ((key, value1): Map.Entry<String, String> in params!!) {
                        ...
                        while (entityFiles != null && i < entityFiles.size) {
                            ...
                            val isLast = (if ((i + 1) == entityFiles.size) true else false)
    ...
    fun uploadFile(ctx: Context, page: String, file: File, total: Long): Int {
        var total = total
        var responseCode = 0
        var connection: HttpURLConnection? = null
        ...
        PreyLogger.d((("page:" + page + " upload:" + file.getName()).toString() + " length:" + file.length()).toString() + " total:" + total)
        try {
            val url = URL(page)
    ...
    companion object {
        ...
        fun getInstance(): UtilConnection {
            if (UtilConnection.INSTANCE == null) {
                UtilConnection.INSTANCE = UtilConnection()
            }
            return UtilConnection.INSTANCE!!
        }
