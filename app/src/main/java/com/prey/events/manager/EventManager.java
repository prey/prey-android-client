...
public class EventManager {
    ...
    public void execute(Event event) {
        ...
        if (Event.WIFI_CHANGED.equals(event.getName())) {
            ...
            if(event.getInfo().indexOf(EventManager.MOBILE)>0){
        ...
        boolean isMobileConnected=PreyConnectivityManager.getInstance(ctx).isMobileConnected();;
        for (int i = 0; isMobileConnected&&i < 10; i++) {
            String networkClass= PreyPhone.getNetworkClass(ctx);
            PreyConfig.getPreyConfig(ctx).setPreviousSsid(networkClass);
            type_connect="mobile";
            break;
        ...
        if (validation) {
            ...
            if (isDeviceRegistered) {
                        PreyLogger.d("EVENT isDeviceRegistered event: "+event.getName()+" type_connect:"+type_connect);
                        this.mapData = new EventMap<String, JSONObject>();
                        this.event = event;
                        if (Event.DEVICE_STATUS.equals(event.getName())) {
                            if(EventManager.MOBILE.equals(type_connect)) {
                        ...
                        if (Event.DEVICE_STATUS.equals(event.getName())) {
                            if(EventManager.MOBILE.equals(type_connect)) {
                                ...
                                new EventRetrieveDataMinWifi().execute(ctx, this);
                            }
                        }else{
                            if(EventManager.MOBILE.equals(type_connect))
                                new EventRetrieveDataMobile().execute(ctx, this);
    ...
    private void sendEvents() {
        if (mapData != null) {
            ...
            if (event != null) {
                    ...
                    if(Event.DEVICE_STATUS.equals(event.getName())){
                        event.setInfo(jsonObjectStatus.toString());
                    }
