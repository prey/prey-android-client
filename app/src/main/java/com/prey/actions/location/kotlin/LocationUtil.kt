...
import com.prey.managers.kotlin.PreyWifiManager
import com.prey.net.kotlin.PreyWebServices
import com.prey.services.kotlin.LocationService
...
object LocationUtil {
    ...
    fun getLocation(
        ...
        try {
            preyLocation =
                getPreyLocationAppService(ctx, method, asynchronous, preyLocation, maximum)
        } catch (e: Exception) {
            PreyLogger.e(String.format("Error PreyLocationApp:%s", e.message), e)
        }
        try {
            if (preyLocation == null || preyLocation.getLocation() == null || (preyLocation.getLat() == 0.0 && preyLocation.getLng() == 0.0)) {
                preyLocation =
    ...
    const val MAXIMUM_OF_ATTEMPTS: Int = 3
    const val MAXIMUM_OF_ATTEMPTS2: Int = 3
    val SLEEP_OF_ATTEMPTS: IntArray = intArrayOf(1, 2, 3, 3)
