...
public class AlarmThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            PreyWebServices.getInstance().sendNotifyActionResultPreyHttp(ctx, UtilJson.makeMapParam("start", "alarm", "started",null,messageId));
            start = true;
            int i = 0;
            ...
            PreyWebServices.getInstance().sendNotifyActionResultPreyHttp(ctx, UtilJson.makeMapParam("start", "alarm", "failed", e.getMessage(),messageId));
        } finally {
            if (mp != null)
                mp.release();
        }
        if (start) {
            PreyWebServices.getInstance().sendNotifyActionResultPreyHttp(ctx, UtilJson.makeMapParam("start", "alarm", "stopped",null,messageId));
        }
